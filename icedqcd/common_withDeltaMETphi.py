...
def splitfactor(x, y, w, ids, args, skip_graph=True, use_dequantize=True):
    ...
    ## Delta phi(MET, muonSV)
    muonSV_phi = np.arctan2(
        data.x['muonSV']['mu1pt'] * np.sin(data.x['muonSV']['mu1phi']) +
        data.x['muonSV']['mu2pt'] * np.sin(data.x['muonSV']['mu2phi']),
        data.x['muonSV']['mu1pt'] * np.cos(data.x['muonSV']['mu1phi']) +
        data.x['muonSV']['mu2pt'] * np.cos(data.x['muonSV']['mu2phi'])
    )
#    data.x['muonSV', 'delta_phi_MET'] = phi_phasewrap(data.x['PFMET_phi'] - muonSV_phi) #v1, no absolute value
    data.x['muonSV', 'delta_phi_MET'] = np.abs(phi_phasewrap(data.x['PFMET_phi'] - muonSV_phi)) #v2, absolute value of angle (distinguishes "pointing the same way" from "pointing opposite ways")
#    data.x['muonSV', 'delta_phi_MET'] = np.minimum(np.abs(phi_phasewrap(data.x['PFMET_phi'] - muonSV_phi)), np.pi - np.abs(phi_phasewrap(data.x['PFMET_phi'] - muonSV_phi))) #v3, smallest angle between lines
    jagged_vars.append('muonSV_delta_phi_MET')
    muonsv_vars.append('muonSV_delta_phi_MET')
