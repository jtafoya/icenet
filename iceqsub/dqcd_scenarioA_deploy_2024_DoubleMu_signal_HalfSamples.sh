...
executable	= tests/runme_dqcd_scenarioA_2024_DoubleMu_deploy_signal_HalfSamples.sh
arguments	= "$(PROCESS) $(NJobs)"
output		= iceqsub/output/outputfile.$(CLUSTER)
error		= iceqsub/error/errorfile.$(CLUSTER)
log		= iceqsub/log/dqcd_scenarioA_deploy_2024_DoubleMu_signal_HalfSamples.job.$(CLUSTER).log
